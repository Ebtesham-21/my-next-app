...
const EditBlog = () => {
  ...
  // ✅ Ensure this is placed before return
  if (!formDataState) {
      ...
      <form onSubmit={handleSubmit} className="grid gap-4">
        ...
        <div>
          ...
          {formDataState.screens && (
            <img src={formDataState.screens} alt="preview" className="w-40 h-24 object-cover rounded" />
          )}
        ...
        <div>
          ...
          {formDataState.blogSingleImg && (
            <img src={formDataState.blogSingleImg} alt="single" className="w-40 h-24 object-cover rounded" />
          )}
          <input type="file" accept="image/*" onChange={handleImageUploadSingle} className="mt-2" />
        </div>

        <button type="submit" className="bg-[#F97316] text-white px-4 py-2 rounded">
          Update Blog
        </button>
      </form>
    </div>
